...
function rcLogo(shop) {
  if (shop.logo) {
    return '<div class="rc-logo"><img src="' + qpEscapeHtml(shop.logo) + '" alt=""></div>';
  }
  if (shop.name) {
    return '<div class="rc-brand">' + qpEscapeHtml(shop.name) + '</div>';
  }
  return '';
}
...
function rcTagline(shop) {
  if (shop.tagline) {
    return '<div class="rc-tagline">' + qpEscapeHtml(shop.tagline) + '</div>';
  }
  return '';
}
...
function rcLabel(text) {
  return '<div class="rc-label">' + qpEscapeHtml(text) + '</div>';
}

function rcValue(text) {
  return '<div class="rc-value">' + qpEscapeHtml(text) + '</div>';
}

function rcRow(label, value) {
  return '<div class="rc-row"><span>' + qpEscapeHtml(label) + '</span><span>' + qpEscapeHtml(value) + '</span></div>';
}

function rcParagraph(text, cls) {
  cls = cls || '';
  return '<div class="rc-paragraph ' + cls + '">' + qpNl2br(text) + '</div>';
}
...
function rcShopFooter(shop, opts) {
  ...
  if (shop.locations && shop.locations.length) {
    if (shop.locations.length === 1) {
      ...
      if (loc.name) html += rcCenter(qpEscapeHtml(loc.name), 'rc-loc-name');
      if (loc.address) html += rcCenter(qpEscapeHtml(loc.address), 'rc-loc-addr');
    } else {
      html += rcDivider('light');
      html += rcLabel(qpT('locationsLabel'));
      shop.locations.forEach(function(loc) {
        var line = loc.name ? (loc.name + ' — ' + loc.address) : loc.address;
        html += '<div class="rc-loc-item">' + qpEscapeHtml(line) + '</div>';
      });
    }
  }
  ...
  if (hasContact) {
    html += rcDivider('light');
    if (shop.email) {
      html += rcLabel(qpT('emailLabel'));
      html += rcValue(shop.email);
    }
    if (shop.phone) {
      html += rcLabel(qpT('phoneLabel'));
      html += rcValue(shop.phone);
    }
    if (shop.whatsapp) {
      html += rcLabel(qpT('whatsappLabel'));
      html += rcValue(shop.whatsapp);
    }
    if (shop.website) {
      html += rcLabel(qpT('website'));
  ...
  if (opts.showOpeningHours && shop.hours) {
    ...
    if (hoursHtml) {
      html += rcDivider('light');
      html += rcLabel(qpT('openingHoursLabel'));
...
function rcWorkerInline(worker) {
  ...
  html += rcCenter(qpEscapeHtml(worker.role));
  html += rcCenter('<strong>' + qpEscapeHtml(worker.name) + '</strong>', 'rc-worker-name');
  if (worker.phone) {
...
var QP_TEMPLATES = {
  discount: {
    ...
    render: function(data, shop, worker) {
      ...
      html += rcCenter('<div class="rc-title">' + qpEscapeHtml(data.headline) + '</div>');
      html += rcDivider('double');

      if (data.code) html += rcCenter('<div class="rc-meta">Code · ' + qpEscapeHtml(data.code) + '</div>');
      if (data.percent) html += rcCenter('<div class="rc-highlight">• ' + qpEscapeHtml(data.percent) + '% OFF •</div>');
      ...
      if (data.validUntil) {
        html += rcCenter('<div class="rc-meta">' + qpT('validUntilLabel') + ' ' + qpFormatDateDMY(data.validUntil) + '</div>');
      }
  ...
  businesscard: {
    ...
    render: function(data, shop, worker) {
      ...
      if (data.tagline) html += rcCenter('<div class="rc-title">' + qpEscapeHtml(data.tagline) + '</div>');
      if (data.role) html += rcCenter('<div class="rc-meta">' + qpEscapeHtml(data.role) + '</div>');
      if (worker) html += rcCenter('<div class="rc-worker-name">' + qpEscapeHtml(worker.name) + '</div>');
  ...
  facial: {
    ...
    render: function(data, shop, worker) {
      var html = rcLogo(shop) + rcTagline(shop);
      html += rcDivider('double');
      if (data.headline) html += rcCenter('<div class="rc-title">' + qpEscapeHtml(data.headline) + '</div>');
      if (data.subheadline) html += rcCenter('<div class="rc-subtitle">' + qpEscapeHtml(data.subheadline) + '</div>');
      html += rcDivider('single');
      ...
      html += rcDivider('single');
      if (data.code) html += rcCenter('<div class="rc-meta">' + qpT('voucherCodeLabel') + ' ' + qpEscapeHtml(data.code) + '</div>');
      if (worker) html += rcCenter('<div class="rc-meta">' + qpT('specialist') + ' ' + qpEscapeHtml(worker.name) + '</div>');
  ...
  skincare: {
    ...
    render: function(data, shop, worker) {
      var html = rcLogo(shop) + rcTagline(shop);
      html += rcDivider('double');
      html += rcCenter('<div class="rc-title">' + qpT('skincarePlan') + '</div>');
      if (data.product) html += rcCenter('<div class="rc-subtitle">' + qpEscapeHtml(data.product) + '</div>');
      html += rcDivider('single');

      if (data.steps) {
        html += rcLabel(qpT('usageLabel'));
        html += qpStepsToHtml(data.steps);
      }

      var meta = '';
      if (data.frequency) meta += rcRow(qpT('frequency'), data.frequency);
      if (data.duration) meta += rcRow(qpT('duration'), data.duration);
      ...
      if (worker) {
        html += rcDivider('single');
        html += rcCenter('<div class="rc-meta">' + qpT('yourSpecialist') + '</div>');
        html += rcCenter('<div class="rc-worker-name">' + qpEscapeHtml(worker.name) + '</div>');
        html += rcCenter('<div class="rc-meta">' + qpEscapeHtml(worker.role) + '</div>');
